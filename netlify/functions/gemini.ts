    ...
    try {
        ...
        switch (action) {
            case 'generateSticker': {
                ...
                if (!responseData) throw new Error("No image data was found in the API response.");
                break;
            }

            case 'editImage': {
                const { base64ImageDataUrl, prompt } = payload;
                if (!base64ImageDataUrl || !prompt) throw new Error('Image data and prompt are required.');
